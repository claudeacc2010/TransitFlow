...
### C.4.1. (Опционально) Верификация email при регистрации — Resend

Без этих переменных регистрация работает мгновенно (авто-подтверждение).
Чтобы новые пользователи подтверждали почту по ссылке из письма:

1. Зарегистрируйся на https://resend.com — кнопка **Sign in with Google**
   (или GitHub), отдельный пароль не нужен.
2. В дашборде: **API Keys** → **Create API Key** → скопируй ключ `re_...`
   (показывается один раз).
3. В Railway сервис **app** → **Variables** добавь:
   - `RESEND_API_KEY` = ключ `re_...`
   - `PUBLIC_BASE_URL` = публичный домен приложения,
     например `https://transitflow-production-3603.up.railway.app`
4. Передеплой (Variables сохраняются → Railway сам передеплоит).

> ⚠️ **Ограничение бесплатного Resend без своего домена:** отправитель
> `onboarding@resend.dev` доставляет письма **только на email владельца
> Resend-аккаунта**. Для демо этого хватает (регистрируйся своей почтой);
> письмо на чужой адрес вернёт ошибку 4xx, и регистрация откатится с
> понятным сообщением. Чтобы слать кому угодно — добавь и верифицируй
> свой домен в Resend (Domains), затем поменяй `RESEND_FROM`.

Проверка: зарегистрируй аккаунт своей почтой → форма скажет «Мы отправили
письмо» → в письме ссылка `/api/auth/verify?token=...` → после клика
редирект на `/login?verified=1` с зелёным баннером → вход работает.
До подтверждения логин отдаёт 403 «Email не подтверждён».      
